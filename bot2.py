...
async def add_movie(interaction: discord.Interaction, movie_name: str):
    ...
    c.execute("INSERT INTO movies (user_id, movie_name, date_added) VALUES (?, ?, ?)", (user_id, movie_name, date_added))
    conn.commit()
...
async def delete_movie(interaction: discord.Interaction, movie_name: str):
    user_id = str(interaction.user.id)
    c.execute("DELETE FROM movies WHERE user_id = ? AND movie_name = ?", (user_id, movie_name))
    conn.commit()
    if c.rowcount > 0:
...
async def clear_movies(interaction: discord.Interaction):
    user_id = str(interaction.user.id)
    c.execute("DELETE FROM movies WHERE user_id = ?", (user_id,))
    conn.commit()
...
async def list_movies(interaction: discord.Interaction):
    user_id = str(interaction.user.id)
    c.execute("SELECT movie_name, date_added FROM movies WHERE user_id = ?", (user_id,))
    movies = c.fetchall()
    if not movies:
...
class MovieView():
    ...
    @bot.command(name='search', description='search for movies')
    async def search(interaction: discord.Interaction, query:str):
        try:
            ...
            with BytesIO() as image_binary:
                ...
                image_binary.seek(0)
                await interaction.followup.send(file=discord.File(fp=image_binary, filename='combined_image.jpg'))
            # Créer l'embed avec les résultats de recherche
            embed = discord.Embed(title="Movie Results")
            view = discord.ui.View()  # Create a View to hold the buttons
            
            for result in results:
                
                # Create a button for each result and add it to the view
                view.add_item(discord.ui.Button(label='Go to ' + result['title'], url=make_room(result['link'])))
            
            
            # Envoyer l'embed avec l'image assemblée
